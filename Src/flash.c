...
void Flash_Read_Obj(uint32_t StartAddrToReadFlash, uint32_t dataAddr, uint32_t length)
{
	...
	{
		*(((uint16_t*)dataAddr)+i) = *((__IO uint16_t*)(StartAddrToReadFlash)+i);
	}
	Flash_Lock();
}
...
void Flash_Write_Obj(uint32_t WriteStartAddr, uint32_t dataAddr, uint32_t length)
{
	...
	uint32_t FlashPgAddr = (WriteStartAddr/FlashPageSize)*FlashPageSize;
	
		//FLASH_Unlock
	Flash_Unlock();
	//Flash_Erase Page
	Flash_Erase(FlashPgAddr);
	//FLASH_Program HalfWord
	for(uint16_t i = 0; i < (length/2);i++)
	{
		FLASH->CR |= FLASH_CR_PG;				/*!< Programming */
...
//		uint16_t data_16 = *(uint16_t*)dataAddr;
		
		*((__IO uint16_t*)(WriteStartAddr)+i) = *(((uint16_t*)dataAddr)+i);
		while((FLASH->SR&FLASH_SR_BSY));
		FLASH->CR &= ~FLASH_CR_PG;
	}
